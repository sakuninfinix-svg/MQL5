//+------------------------------------------------------------------+
//| Signal/RegimeFilter.mqh — v1.00  (Phase 4)                       |
//| Volatility regime detection: TRENDING / RANGING / VOLATILE       |
//|                                                                  |
//| ALGORITHM:                                                       |
//|  Uses three indicators in combination:                           |
//|  1. ADX: trend strength (>25 = trending, <20 = ranging)          |
//|  2. ATR percentile: compare current ATR vs rolling median        |
//|     (ATR > 1.8x median = volatile)                               |
//|  3. Bollinger Width: band squeeze detection                       |
//|     (BW < 0.5x median = squeeze / compressed ranging)            |
//|                                                                  |
//| REGIME RULES (priority order):                                   |
//|  VOLATILE  : ATR > atrVolatileRatio * median  (dangerous)        |
//|  TRENDING  : ADX >= adxTrendThreshold AND ATR not volatile       |
//|  SQUEEZE   : BW < bwSqueezeRatio * median  (pre-breakout)        |
//|  RANGING   : default                                             |
//+------------------------------------------------------------------+
#property strict
...
enum ENUM_MARKET_REGIME
  {
   REGIME_TRENDING  = 0,   // ADX strong, directional
   REGIME_RANGING   = 1,   // Low ADX, mean-reverting
   REGIME_VOLATILE  = 2,   // ATR spike, unpredictable
   REGIME_SQUEEZE   = 3,   // BB width compressed, coiled
   REGIME_UNKNOWN   = 4    // Not enough data
  };
...
#define RF_HISTORY        50    // bars to compute rolling median
...
private:
   ...
   int  m_hBBU;   // BB upper
   int  m_hBBL;   // BB lower
   int  m_hBBM;   // BB middle (actually same handle — we copy bands)
   int  m_hBB;    // single BB handle, we copy all bands from it

   // Thresholds (configurable)
   double m_adxTrendThreshold;  // default 25
   double m_atrVolatileRatio;   // default 1.8  (current ATR > ratio * median)
   double m_bwSqueezeRatio;     // default 0.5  (BW < ratio * median = squeeze)
   ...
   double  m_currentATR;
   double  m_atrMedian;
   double  m_currentBW;    // Bollinger Width = (upper-lower)/middle
   double  m_bwMedian;
   bool    m_ready;

   // Rolling history for median
   double  m_atrHistory[RF_HISTORY];
   double  m_bwHistory[RF_HISTORY];
   int     m_histIdx;
   int     m_histFilled;

   // ── Median computation (simple sort-based) ───────────────────
   double CalcMedian(double &arr[], int n) const
     {
      if(n <= 0) return 0;
      double tmp[]; ArrayResize(tmp, n);
      ArrayCopy(tmp, arr, 0, 0, n);
      // Partial insertion sort sufficient for small n
      for(int i=1; i<n; i++)
        {
...
public:
   ...
   virtual void OnNewBar() override
     {
      // Copy indicator values (bar 1 = last closed)
      ...
      if(m_histFilled >= 10)  // need at least 10 bars of history
        {
      ...
      if(m_debugMode && m_regime != prev)
         PrintFormat("[Regime] %s → %s | ADX=%.1f ATR=%.5f(x%.1f median) BW=%.3f",
                     RegimeName(prev), RegimeName(m_regime),
                     m_currentADX,
                     m_currentATR, m_atrMedian>0?m_currentATR/m_atrMedian:0,
                     m_currentBW);
     }
