...
bool IsBounce(bool buy)
{
   ...
   return nearSupport && (upperWick >= dynamicWickRatio * body) && (c[0] < c[1]) && c[0] < k[0] && c[0] < t[0];
}
...
bool IsInTradingSession()
{
   ...
   int s1s = (int)StringToInteger(StringSubstr(Session1Start, 0, 2)) * 60 + (int)StringToInteger(StringSubstr(Session1Start, 3, 2));
   int s1e = (int)StringToInteger(StringSubstr(Session1End, 0, 2)) * 60 + (int)StringToInteger(StringSubstr(Session1End, 3, 2));
   int s2s = (int)StringToInteger(StringSubstr(Session2Start, 0, 2)) * 60 + (int)StringToInteger(StringSubstr(Session2Start, 3, 2));
   int s2e = (int)StringToInteger(StringSubstr(Session2End, 0, 2)) * 60 + (int)StringToInteger(StringSubstr(Session2End, 3, 2));
...
void UpdateStatus(string msg)
{
   ...
   string softLoss = (todayProfit <= -AccountInfoDouble(ACCOUNT_BALANCE) * SoftLossThresholdPercent / 100.0) ? "🔴SOFT LOSS" : "🟢NORMAL";
