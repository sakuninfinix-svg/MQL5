...
input double   InpMinATR           = 10.0;       // Min ATR (Points) untuk menghindari Choppy
...
input group "== S&R & Price Action =="
input int      InpSRLookback       = 20;         // Lookback Candle untuk S&R M5
...
input bool     InpUseHedging       = true;       // Aktifkan Hedging jika Breakout
input double   InpHedgeMult        = 1.5;        // Multiplier Lot Hedge

input group "== System =="
input long     InpMagicNum         = 20260403;   // Magic Number
...
void OnTick() {
   ...
   // 1. Basic Filters
   if(!IsNewCandle()) return; // Cek per candle untuk efisiensi
...
void CheckEntrySignals() {
   ...
   if(wickType == 1 && price <= targetSupport + (InpBufferPoints * _Point)) {
      ...
      if(trade.Buy(InpLotSize, _Symbol, price, sl, tp, "Scalp Buy")) {
         if(InpSendPush) SendNotification("EA Scalp: Buy Executed at Support");
      }
   }
   ...
   if(wickType == -1 && price >= targetResistance - (InpBufferPoints * _Point)) {
      ...
      if(trade.Sell(InpLotSize, _Symbol, price, sl, tp, "Scalp Sell")) {
         if(InpSendPush) SendNotification("EA Scalp: Sell Executed at Resistance");
      }
   }
}
