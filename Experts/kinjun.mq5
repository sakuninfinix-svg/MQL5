...
int OnInit()
{
   ...
   if(hIchimoku == INVALID_HANDLE || hKijun == INVALID_HANDLE || hTenkan == INVALID_HANDLE || hATR == INVALID_HANDLE)
      return INIT_FAILED;
...
void CheckLayering()
{
   ...
   if(PyramidingMode == PROFIT)
   {
      ...
      if(PositionGetDouble(POSITION_PROFIT) <= 0.0) return;
   }
   ...
   if(tradeDir == 0 && trade.Buy(lot, _Symbol, 0.0, 0.0, 0.0)) { pLevel++; lastEntry = currentPrice; }
   else if(tradeDir == 1 && trade.Sell(lot, _Symbol, 0.0, 0.0, 0.0)) { pLevel++; lastEntry = currentPrice; }
}
...
void ManageExitLogic()
{
   ...
   if(currDist >= slDist * 0.8 && (currSl == 0.0 || MathAbs(currSl - openPrice) > 10 * _Point))
   {
      double nSl = openPrice + (tradeDir == 0 ? BEP_Buffer_Points * _Point : -BEP_Buffer_Points * _Point);
      trade.PositionModify(_Symbol, nSl, 0.0);
   }
   ...
   if(UseTrailingKijun && currDist > slDist * 1.5)
   {
      ...
      if(tradeDir == 0 && kijun[0] > currSl) trade.PositionModify(_Symbol, kijun[0], 0.0);
      if(tradeDir == 1 && (kijun[0] < currSl || currSl == 0.0)) trade.PositionModify(_Symbol, kijun[0], 0.0);
   }
}
...
double CalculateLot(double slPoints)
{
   ...
   if(slPoints <= 0.0 || tv <= 0.0 || ts <= 0.0 || pt <= 0.0) return 0.0;
   double lot = risk / (MathMax(slPoints, 10.0) * (tv / (ts / pt)));
...
void CheckInitialEntry()
{
   ...
   if(IsBullish() && IsBounce(true))
   {
      ...
      if(trade.Buy(lot, _Symbol, 0.0, sl, 0.0)) { pLevel = 1; tradeDir = 0; lastEntry = ask; }
   }
   else if(IsBearish() && IsBounce(false))
   {
      double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double sl = bid + (atr[0] * SL_ATR_Multiplier);
      if(trade.Sell(lot, _Symbol, 0.0, sl, 0.0)) { pLevel = 1; tradeDir = 1; lastEntry = bid; }
   }
}
...
bool IsInTradingSession()
{
   ...
   return (m >= 120 && m <= 360) || (m >= 540 && m <= 900);
}
...
int CountPos()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket != 0 && PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == MagicNumber) c++;
   }
   return c;
}
...
void CloseAll()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(t != 0 && PositionSelectByTicket(t) && PositionGetInteger(POSITION_MAGIC) == MagicNumber)
      {
         if(PositionGetDouble(POSITION_PROFIT) < 0.0) { lastFailedDir = (int)PositionGetInteger(POSITION_TYPE); failedBlacklistBar = 3; }
