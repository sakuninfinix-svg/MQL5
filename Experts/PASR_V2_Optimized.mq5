...
#include <MQL5/Include/MarketManager.mqh> // Include the new MarketManager class
#include <MQL5/Include/SRManager.mqh>     // Include the new SRManager class
CTrade trade;

enum ENUM_ENTRY_MODE { MODE_SAFE, MODE_AGGRESSIVE };
enum ENUM_SR_MODE    { SR_EXTREME, SR_SWING };
enum ENUM_EXIT_MODE  { EXIT_SR_TARGET, EXIT_FIX_TP, EXIT_PARTIAL_LOCK };
...
input ENUM_ENTRY_MODE InpEntryMode = MODE_SAFE;
...
input double InpMinWickRatio = 45.0;
...
input int    InpEntryCooldownBars = 1;
...
input group "== Exit & Trailing Strategy =="
input ENUM_EXIT_MODE InpExitMode = EXIT_SR_TARGET;
...
input group "== Adaptive Trailing (ATR) =="
input double InpTrailActivationATR = 1.8;
input double InpTrailStepATR = 0.7;
input double InpLockProfitATR = 1.2;
input double InpLockLevelATR = 0.3;

input group "== Advanced Hedging Recovery =="
input bool   InpUseHedging = false;
...
input group "== Self Recovery Trade =="
...
input group "== Partial Close Settings (Virtual TP) =="
...
input group "== System =="
input long InpMagicNum = 20260403;
...
int OnInit() {
   ...
   for(int i=0; i<10; i++) tradeEngines[i].Reset();
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      if(HistoryDealSelect(trans.deal))
      {
         ...
         if(magic == InpMagicNum && symbol == _Symbol && (entryType == DEAL_ENTRY_OUT || entryType == DEAL_ENTRY_INOUT))
         {
            ...
            if(StringFind(comment, "HEDGE") >= 0) return;
...
void OnTick()
{
   ...
   sr.UpdateMainZones();
   sr.UpdateHTFZones();
   if(!sr.IsTradableRange(market.GetATRPoints())) { statSRRange++; return; }
   if(market.IsDailyLossLimitHit()) return;
   if(IsEntryCooldownActive()) return;
   if(posScan.normalCount >= InpMaxOpenPositions) return;
...
bool IsValidRejection(int shift, double &wickType, double &signalPrice, bool &midpointFail)
{
   ...
   if(range > InpMaxSignalSizeATR * atr) return false;
...
{
   if(!InpOneEntryPerZone) return false;
...
void CheckEntrySignals()
{
   ...
   for(int shift = 1; shift <= InpSignalLookback; shift++)
   {
      ...
      if(midFail) midFailFound = true;
   }

   if(signalShift < 0)
   {
      if(midFailFound) statMidpoint++;
   ...
   {
      if(IsZoneReuseBlocked(true, sr.Support())) return;
      ExecuteEntry(ORDER_TYPE_BUY, signalPrice);
   }
   else if(wickType == -1.0 && nearResistance)
   {
      if(IsZoneReuseBlocked(false, sr.Resistance())) return;
...
{
   double bid, ask;
   if(!GetCurrentPrices(bid, ask)) return;
   ...
   if(type == ORDER_TYPE_BUY && InpBlockCounterHTF && mtfBias < 0) { statHTF++; return; }
   if(type == ORDER_TYPE_SELL && InpBlockCounterHTF && mtfBias > 0) { statHTF++; return; }
   ...
   double tp = (InpExitMode == EXIT_FIX_TP) ? 
               (type == ORDER_TYPE_BUY ? NormalizeDouble(ask + (atr * InpFixedTPATR), _Digits) : NormalizeDouble(bid - (atr * InpFixedTPATR), _Digits)) :
               (type == ORDER_TYPE_BUY ? NormalizeDouble(sr.Resistance(), _Digits) : NormalizeDouble(sr.Support(), _Digits));
   ...
   if(result)
   {
      ...
      if(ticket == 0) ticket = trade.ResultDeal(); // Safety for some brokers
...
{
   if(!r.active) return;
   if(!PositionSelectByTicket(r.mainTicket)) { r.Reset(); return; }
      ...
      case TRADE_STATE_VSL_HIT:
         {
            ...
            if(isTooClose)
            {
               if(r.direction > 0) r.recoveryTP = NormalizeDouble(curPrice + stopLevel + _Point, _Digits);
               else r.recoveryTP = NormalizeDouble(curPrice - stopLevel - _Point, _Digits);
            }
            ...
            else
            {
               ...
               else market.DebugLog("VSL_HIT: Modify failed. Code: " + (string)retCode);
            }
            break;
         }
...
{
   for(int i=0; i<10; i++) if(tradeEngines[i].active && tradeEngines[i].state == TRADE_STATE_HEDGE_ACTIVE) return true;
   return false;
}
...
bool ValidateOrderLevels(ENUM_ORDER_TYPE type, double price, double sl, double tp, string &reason)
{
   ...
   if(type == ORDER_TYPE_BUY)
   {
      if(tp <= price) { reason = "BUY TP invalid"; return false; }
      if(sl >= price) { reason = "BUY SL invalid"; return false; }
      if(tp - price < requiredTP) { reason = "BUY TP too close"; return false; }
      if(price - sl < stopLevel) { reason = "BUY SL too close"; return false; }
   }
   else
   {
      if(tp >= price) { reason = "SELL TP invalid"; return false; }
      if(sl <= price) { reason = "SELL SL invalid"; return false; }
      if(price - tp < requiredTP) { reason = "SELL TP too close"; return false; }
      if(sl - price < stopLevel) { reason = "SELL SL too close"; return false; }
   }
...
void ScanAndProcessPositions(const MqlTick &tick)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      {
         posScan.normalCount++;
         if(posType == POSITION_TYPE_BUY) posScan.buyCount++;
         if(posType == POSITION_TYPE_SELL) posScan.sellCount++;
      }
      ...
      if(!isHedge)
      {
         ...
         if(engineIdx != -1)
         {
            if(tradeEngines[engineIdx].state == TRADE_STATE_NORMAL) ApplyNormalTrailing(ticket, tick);
...
{
   for(int i=0; i<10; i++) if(tradeEngines[i].active && tradeEngines[i].mainTicket == ticket) return i;
   return -1;
}
...
{
   if(!PositionSelectByTicket(sourceTicket)) return 0.0;
...
void TryActivateHedge(RecoveryEngine &r)
{
   ...
   if(OpenHedgeOrder((r.direction > 0 ? POSITION_TYPE_BUY : POSITION_TYPE_SELL), hedgeLot, hedgeSL, r.mainTicket))
   {
      ...
      if(r.hedgeTicket == 0) r.hedgeTicket = trade.ResultDeal();
...
{
   if(PositionSelectByTicket(r.mainTicket)) trade.PositionClose(r.mainTicket);
   if(PositionSelectByTicket(r.hedgeTicket)) trade.PositionClose(r.hedgeTicket);
   
   if(reason == "RECOVERY_TARGET" || reason == "RECOVERY_BE_PULLBACK") statHedgeSave++;
   else statHedgeFail++;
