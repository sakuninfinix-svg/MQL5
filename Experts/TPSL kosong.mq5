...
#property version "2.09 Magic + ATR Full"
#property strict
...
int CalculateCurrentPositions()
{
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if (MagicNumber != 0 && pos_magic != MagicNumber)
         continue;
      if (ChartSymbolSelection == CurrentChartSymbol && PositionGetString(POSITION_SYMBOL) != _Symbol)
         continue;
...
{
   if (LockProfitEnable == LP_DISABLE || target_points == 0 || locked_points == 0)
      return false;
...
bool TrailingStopFunc(ulong ticket, int trail_points, int step, ENUM_TRAILINGSTOP_METHOD method)
{
   ...
   if (profit_points > trail_points)
   {
      if (ptype == POSITION_TYPE_BUY && (bid - current_sl) / pt >= trail_points)
      {
         ...
         {
         case TS_CLASSIC:
            tsl = NormalizeDouble(bid - trail_points * pt, dig);
            break;
         case TS_STEP_DISTANCE:
            tsl = NormalizeDouble(bid - (trail_points - step) * pt, dig);
            break;
         case TS_STEP_BY_STEP:
            tsl = NormalizeDouble(current_sl + step * pt, dig);
            break;
         }
      }
      else if (ptype == POSITION_TYPE_SELL && (current_sl - ask) / pt >= trail_points)
      {
         ...
         {
         case TS_CLASSIC:
            tsl = NormalizeDouble(ask + trail_points * pt, dig);
            break;
         case TS_STEP_DISTANCE:
            tsl = NormalizeDouble(ask + (trail_points - step) * pt, dig);
            break;
         case TS_STEP_BY_STEP:
            tsl = NormalizeDouble(current_sl - step * pt, dig);
            break;
...
void OnTick()
{
   if (Bars(_Symbol, PERIOD_CURRENT) < 100 || !TerminalInfoInteger(TERMINAL_TRADE_ALLOWED))
      return;
