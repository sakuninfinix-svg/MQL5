...
input bool               BlockMediumImpact = true;
...
void OnTick()
{
   ...
   if(myPos == 0) { pLevel = 0; tradeDir = -1; lastEntry = 0.0; }
   ...
   if(myPos >= GlobalMaxPositions) { UpdateDashboard("MAX POS"); return; }
   if(GetSpread() > MaxSpread || !IsInTradingSession()) { UpdateDashboard("FILTER"); return; }
...
void CheckInitialEntry()
{
   ...
   if(CopyLow(_Symbol, LowerTimeframe, 1, 1, l) <= 0) return;
   if(CopyHigh(_Symbol, LowerTimeframe, 1, 1, h) <= 0) return;
   ...
   if(IsBullish() && IsBounce(true))
   {
      ...
      if(trade.Buy(lot, _Symbol, 0.0, sl, 0.0))
      {
   ...
   else if(IsBearish() && IsBounce(false))
   {
      ...
      if(trade.Sell(lot, _Symbol, 0.0, sl, 0.0))
      {
...
void CheckLayering()
{
   ...
   {
      if(!PositionSelect(_Symbol) || PositionGetDouble(POSITION_PROFIT) <= 0.0) return;
   }
...
void ManageExitLogic()
{
   ...
   {
      trade.PositionModify(_Symbol, openPrice + (tradeDir == 0 ? 7 * _Point : -7 * _Point), 0.0);
      return;
   }
   ...
   if(currDist >= slDist * 1.1)
   {
      ...
      {
         if(tradeDir == 0) trade.Sell(closeVol);
         else trade.Buy(closeVol);
      }
   }
   ...
   {
      double kijun[1];
      if(CopyBuffer(hKijun, 0, 0, 1, kijun) <= 0) return;
      if(tradeDir == 0 && kijun[0] > currSl) trade.PositionModify(_Symbol, kijun[0] - 3 * _Point, 0.0);
      if(tradeDir == 1 && kijun[0] < currSl) trade.PositionModify(_Symbol, kijun[0] + 3 * _Point, 0.0);
   }
}
...
bool IsBounce(bool buy)
{
   ...
   double atrAvg[20];
   if(CopyBuffer(hATR, 0, 0, 20, atrAvg) < 20) return false;
   ...
   if(buy) return nearSupport && (lowerWick >= dynamicWickRatio * body) && (c[0] > c[1]) && c[0] > k[0] && c[0] > t[0] && IsVolumeConfirmed();
   return nearSupport && (upperWick >= dynamicWickRatio * body) && (c[0] < c[1]) && c[0] < k[0] && c[0] < t[0] && IsVolumeConfirmed();
}
...
double CalculateLot(double slPoints)
{
   ...
   if(slPoints <= 0.0 || tv <= 0.0) return 0.0;
   double lot = riskAmount / (slPoints * tv);

   double lossThreshold = balance * (SoftLossThresholdPercent / 100.0) * -1.0;
...
int CountPos()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket != 0 && PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == MagicNumber && PositionGetString(POSITION_SYMBOL) == _Symbol) count++;
   }
   return count;
}
...
bool BasketCloseCheck()
{
   ...
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket != 0 && PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == MagicNumber && PositionGetString(POSITION_SYMBOL) == _Symbol)
...
void CloseAll()
{
   ...
   if(todayProfit < 0.0) { lastFailedDir = tradeDir; failedBlacklistBar = 3; winStreak = 0; lossStreak++; }
   else { lossStreak = 0; winStreak++; }

   pLevel = 0; tradeDir = -1; lastEntry = 0.0;
}
...
bool IsInTradingSession()
{
   ...
   int s1s = StringToMinutes(Session1Start);
   int s1e = StringToMinutes(Session1End);
   int s2s = StringToMinutes(Session2Start);
   int s2e = StringToMinutes(Session2End);
...
{
   long vol[5];
   if(CopyTickVolume(_Symbol, LowerTimeframe, 1, 5, vol) < 5) return false;
   long avgVol = (vol[1] + vol[2] + vol[3] + vol[4]) / 4;
   return vol[0] >= avgVol * 1.2;
}

void UpdateDashboard(string msg)
{
   string bias = IsBullish() ? "🟢BULL" : IsBearish() ? "🔴BEAR" : "⚪NEUTRAL";
   string text = StringFormat(
      "🚀 %s | Lvl:%d/%d | Bias:%s | Pos:%d/%d\n"
      "💰 Today: $%.1f | Spread:%.0f | %s",
      EA_Name, pLevel, MaxPyramidingLevel, bias, CountPos(), GlobalMaxPositions,
      todayProfit, GetSpread() / _Point, msg
   );
   Comment(text);
}
