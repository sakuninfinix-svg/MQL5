#!/usr/bin/env python3
"""
train_entry_quality_weights.py — Train/export PASR entry quality weights

Expected CSV columns:
  label,f0,f1,f2,f3,f4,f5,f6,f7[,weight]

label:
  1 = entry was good / profitable / acceptable drawdown
  0 = entry was poor / noisy / invalid
...
Feature contract:
  ...
  f4 = rrNorm
  f5 = spreadQuality
  f6 = volatilityQuality
  f7 = sessionQuality

Output binary layout, float32:
  [magic, version, n_features=8, bias, w0..w7]
"""
...
DEFAULT_WEIGHTS = [-3.00, 1.25, 1.35, 0.85, 1.10, 1.20, 0.70, 0.60, 0.55]
FEATURE_NAMES = ["patternScore", "zoneScore", "regimeConfidence", "aiScore", "rrNorm", "spreadQuality", "volatilityQuality", "sessionQuality"]
...
def export_binary(weights: list[float], out_path: str) -> None:
    ...
    print(f"[train_entry_quality_weights] wrote {out_path}")
...
def main() -> int:
    ...
    export_binary(weights, str(out_dir / "PASR_entry_quality_weights.bin"))
    report = {
    ...
    with open(out_dir / "entry_quality_training_report.json", "w", encoding="utf-8") as f:
