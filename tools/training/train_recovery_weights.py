#!/usr/bin/env python3
"""
train_recovery_weights.py — Train/export PASR recovery eligibility weights

Expected CSV columns:
  label,f0,f1,f2,f3,f4,f5,f6[,weight]

label:
  1 = recovery/re-entry was justified
  0 = recovery should be avoided
...
DEFAULT_WEIGHTS = [-2.85, -1.10, 1.35, 1.20, 0.95, 0.70, 1.40, 0.90]
FEATURE_NAMES = ["lossNorm", "zoneQuality", "patternRecovery", "regimeStability", "spreadQuality", "drawdownSafety", "retrySafety"]
...
def export_binary(weights: list[float], out_path: str) -> None:
    ...
    print(f"[train_recovery_weights] wrote {out_path}")
...
def main() -> int:
    ...
    export_binary(weights, str(out_dir / "PASR_recovery_weights.bin"))
    report = {
    ...
    with open(out_dir / "recovery_training_report.json", "w", encoding="utf-8") as f:
